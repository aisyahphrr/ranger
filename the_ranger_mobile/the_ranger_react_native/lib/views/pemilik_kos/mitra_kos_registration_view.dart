...
class MitraKosRegistrationView extends StatefulWidget {
  const MitraKosRegistrationView({super.key});

  @override
  State<MitraKosRegistrationView> createState() => _MitraKosRegistrationViewState();
}

class _MitraKosRegistrationViewState extends State<MitraKosRegistrationView> {
  int _currentStep = 0;
  ...
  Future<void> _nextStep() async {
    if (_currentStep == 0) {
      ...
      if (_formKey.currentState?.validate() ?? false) {
        ...
        await provider.registerKosOwner(
        ...
        if (mounted) provider.navigate(AppScreen.kosOwnerSuccess);
      }
    }
  }
  ...
  Widget _buildTitleSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: const [
        Text('Daftar Pemilik Kos', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: AppColors.textPrimary)),
        SizedBox(height: 6),
        Text('Lengkapi data untuk membuat akun pemilik kos Anda.', style: TextStyle(color: AppColors.textSecondary))
      ],
    );
  }
                  ...
                  child: ListView(
                    ...
                    children: [
                      if (_currentStep == 0) ...[
                        ...
                        _buildSectionCard(
                          title: 'Detail Usaha',
                          subtitle: 'Lengkapi informasi usaha kos Anda.',
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.stretch,
                            children: [
                              _buildTextField(
                                ...
                                hintText: 'Nama Kos Anda (Cth: Kos Putri Melati)',
                                validator: (value) => value == null || value.isEmpty ? 'Harus diisi' : null,
                              ),
                              const SizedBox(height: 16),
                              _buildTextField(
